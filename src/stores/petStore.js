...
export const usePetStore = defineStore('pet', () => {
  ...
  const lastViewedPetForSnapshot = ref(null)
  const userStore = useUserStore()

  const hydrateFromSnapshot = (snapshot) => {
    if (!snapshot?.pets || !Array.isArray(snapshot.pets)) return false
    pets.value = snapshot.pets
    if (snapshot.lastViewedPet) {
      lastViewedPetForSnapshot.value = snapshot.lastViewedPet
    }
    return true
  }
